...
public class JdbiFactory {
  ...
  /**
   * Build a fully configured {@link Jdbi} instance managed by the DropWizard lifecycle with the
   * configured health check; this method should not be overridden (instead, override {@link
   * #newInstance(ManagedDataSource)} and {@link #configure(Jdbi)})
   *
   * @param environment
   * @param configuration
   * @param name
   * @return A fully configured {@link Jdbi} object using a managed data source based on the
   *     specified environment and configuration
   * @see #build(Environment, PooledDataSourceFactory, ManagedDataSource, String)
   */
  ...
  /**
   * This creates a default {@link InstrumentedSqlLogger} instance with the specified {@link
   * MetricRegistry} and {@link #nameStrategy}. This can be overridden if required.
   *
   * @param metricRegistry The {@link MetricRegistry} to send to the {@link InstrumentedSqlLogger}.
   * @param nameStrategy The {@link StatementNameStrategy} to send to the {@link
   *     InstrumentedSqlLogger}.
   * @return The created {@link InstrumentedSqlLogger}.
   * @since 2.0
   */
  protected InstrumentedSqlLogger buildSQLLogger(
      MetricRegistry metricRegistry, StatementNameStrategy nameStrategy) {
    return new InstrumentedSqlLogger(metricRegistry, nameStrategy);
  }

  /**
   * This creates a vanilla {@link Jdbi} instance based on the specified data source; this can be
  ...
  /**
   * Overridable function to allow extra customization of the created {@link Jdbi} instance.
   *
   * <p>If this is overridden it is strongly recommend that {@code super.configure(jdbi,
   * configuration)} is invoked before any other changes are made if you intend to use the default
   * as a base so that the customized settings will supersede the defaults
   *
   * @param jdbi
   */
